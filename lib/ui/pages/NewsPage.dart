...
class _NewsPageState extends State<NewsPage> {
  ...
  Widget actionWidget(BuildContext context) {
    return
      new Image(image: FirebaseImage(_eventsInfo["imageAction"]));
  }
...
//      return PhoneLogin();
//    }
    return  Scaffold(body: buildNewsPage(context),appBar: buildAppBar(context),);
  }


  void _handleTap(String documentID, String title, String description, String image, String date, String like) {
    setState(() {
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => NewsDetailPage(documentID, title, description, image, date, like, _result[documentID]),
        ),
      );
    });
  }
  ...
  void _likeHandleTap(DocumentSnapshot document) async {
    ...
    setState(() {
      if(_result[document.id]) {// уже сделал лайк
        _result[document.id] = false;
        store
            .collection("news")
            .doc(document.id)
            .update(data: Map.from({"like" : FieldValue.arrayRemove(List.unmodifiable([getUserId()]))}));
        ...
        store
            .collection("news")
            .doc(document.id)
            .update(data: Map.from({"like" : FieldValue.arrayUnion(List.unmodifiable([getUserId()]))}));
            ...
            StreamBuilder<QuerySnapshot>(
              ...
              builder: (BuildContext context, AsyncSnapshot<QuerySnapshot> snapshot) {
                ...
                switch (snapshot.connectionState) {
                  ...
                  default:
                    Map<String, bool> result = new Map();
                    snapshot.data.docs.forEach((doc) => result.putIfAbsent(
                        doc.id, () => List.from(doc.data()['like']).contains(getUserId())
                      //     () => debugPrint("likesResultForEach:" + List.from(doc['like']).toString() +" " + _user.uid) as bool
                    ));
                    _result = result;
                    return new ListView.builder(
                        ...
                        itemBuilder: (BuildContext ctx, int index) {
                          return  GestureDetector(
                            child: buildMYColumn(document: snapshot.data.docs.elementAt(index)),
                            onTap:() {
                              _handleTap(
                                  snapshot.data.docs.elementAt(index).id,
                                  snapshot.data.docs.elementAt(index).data()['title'].toString(),
                                  snapshot.data.docs.elementAt(index).data()['description'].toString(),
                                  snapshot.data.docs.elementAt(index).data()['image'].toString(),
                                  snapshot.data.docs.elementAt(index).data()['date'].toString(),
                                  List.from(snapshot.data.docs.elementAt(index).data()['like']).length.toString()
  ...
  Widget buildMYColumn({DocumentSnapshot document}) {
    ...
    return buildMyCardWithPaddingNotOnTap(
        Column(
            ...
            children: <Widget>[
              Padding(
                ...
                child: Container(
                    ...
                    child: (imageUrl ==null || imageUrl.isEmpty)  ? Container() : MyImageWidget(url: imageUrl)),
              ),
              ...
              new Row (
                  ...
                  children: <Widget>[
                   new GestureDetector(
                    ...
                    new Text(
                      formatDate(DateTime.fromMillisecondsSinceEpoch(int.parse(document.data()['date'])), [  dd, '.', mm, '.', yyyy]),
                      style: new TextStyle(fontSize:14.0,
                          color: const Color(0xFF000000),
                          fontWeight: FontWeight.w300,
                          fontFamily: "Roboto"),
